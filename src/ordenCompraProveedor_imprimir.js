...
const imprimir = async (req, res) => {
  try {
    ...
    if (!ordenCompra) {
      return res.status(404).json({ ok: false, msg: "Orden de compra no encontrada" });
    }
    ...
    res.setHeader("Content-Disposition", `inline; filename=OrdenCompra_${id}.pdf`);
    ...
    doc.font("Helvetica-Bold")
      .fontSize(20)
      .text("Orden de Compra Proveedor", 50, 50, { align: "center" });

    // Texto a la derecha: fecha y código
    const fechaOrdenCompra = new Date(ordenCompra.fecha).toLocaleDateString();
    const codigoOrdenCompra = ordenCompra._id?.toString().slice(-6) || "N/A";
    const presupuesto = ordenCompra.presupuesto;

    doc.font("Helvetica")
      .fontSize(10)
      .text(`Fecha: ${fechaOrdenCompra}\nN° Orden: ${codigoOrdenCompra}\nN° Presup.: ${presupuesto?._id || "N/A"}`, 490, 55, {
        align: "left",
        width: 150
      });
...
// ---------------- Datos del NotaPedido, cliente y empleado ----------------
    ...
    const cDatosProveedor = [
      ["Código:", ordenCompra.proveedor?._id || "N/A"],
      ["Nombre:", ordenCompra.proveedor?.name || "N/A"],
      ["CUIT:", ordenCompra.proveedor?.cuit || "N/A"],
      ["Email:", ordenCompra.proveedor?.email || "N/A"],
      ["Teléfono:", ordenCompra.proveedor?.telefono || "N/A"],
      ["País:", ordenCompra.proveedor?.pais?.name || "N/A"],
      ["Provincia:", ordenCompra.proveedor?.provincia?.name || "N/A"],
      ["Localidad:", ordenCompra.proveedor?.localidad?.name || "N/A"],
      ["Barrio:", ordenCompra.proveedor?.barrio?.name || "N/A"],
      ["Calle:", ordenCompra.proveedor?.calle?.name || "N/A"],
      ["Altura:", ordenCompra.proveedor?.altura || "N/A"],
      ["Cond. IVA:", ordenCompra.proveedor?.condicionIva?.name || "N/A"],
    ];

    // Empleado
    const cDatosEmpleado = [
      ["Código:", ordenCompra.empleado?._id || "N/A"],
      ["Nombre:", ordenCompra.empleado?.name || "N/A"],
      ["Apellido:", ordenCompra.empleado?.lastname || "N/A"],
      ["CUIT:", ordenCompra.empleado?.cuit || "N/A"],
    ];
    ...
    // ---------------- Medio de Pago ----------------
    const medioPagoNombre = ordenCompra.medioPago?.name || "N/A";
    doc.font("Helvetica-Bold").fontSize(10)
      .text("Medio de Pago:", 50, cSepYFin + 20);
    doc.font("Helvetica").fontSize(10)
      .text(medioPagoNombre, 125, cSepYFin + 20);

    // Línea horizontal debajo
    doc.moveTo(50, cSepYFin + 40)
      .lineTo(550, cSepYFin + 40)
      .stroke();
  ...
  ordenCompra.detalles.forEach((det, i) => {
    const rowY = doc.y;
    ...
    doc.font("Helvetica-Bold")
      .fontSize(16)
      .text(`TOTAL: $${ordenCompra.total}`, 50, totalY, { align: "right" });
